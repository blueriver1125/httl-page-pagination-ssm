	...
	return this.each( function() {
		...
		var ts= this, grid={
			...
			dragEnd: function() {
				...
				//add by cyguo 宽度保存在cookie中
				var json = $.cookie("jqgrid_col_width");
				var col_width = {};
				if(json!=null&&$.trim(json) != ''){
					col_width = JSON.parse(json);
				}
				col_width[idx] = nw;
				json = JSON.stringify(col_width);
				$.cookie("jqgrid_col_width", json, {expires: 7, path: '/'});
			},
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			var tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
				...
				//add by cyguo
				var matchStateA = cur['matchStateA'];
				var matchStateM = cur['matchStateM'];
				var auditState = cur['auditState'];
				if(auditState=="2"){
					classes += ' hight-light-green';
				}else{
					if(matchStateA=="2"||matchStateM=="2"){
						classes += ' hight-light-yellow';
					}
					var matchStdId = cur['matchStdId'];
					if(matchStdId=="UNABLEMATCH"){
						classes += ' hight-light-red';
					}
				}
...
//		$(eg).css("width",grid.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");
		$(eg).css("width","100%").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");
...
//			$(ts.p.pager).css({width: grid.width+"px"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
			$(ts.p.pager).css({width: "100%"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
...
//			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: (grid.width)+"px"})
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: "100%"})
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						//add by cyguo 
						var index_cyguo = postd[idname];
						postd['f1'] = $($t).find("#"+index_cyguo+"_f1").val();
